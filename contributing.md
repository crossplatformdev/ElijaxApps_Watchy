...
Check [Current Verification](docs/readme.md#current-verification) before
interpreting these commands: the current tree has a strict-warning test
failure; the gallery synchronizer now uses the correct SDK source path.
Run and report the checks even when blocked; do not describe them as passing
or update golden hashes to hide a mismatch. Native replay/timing checks also
require a host C++ compiler and may otherwise be skipped.

## Project rules

- Begin complete screens with `WatchyUi::Screen::beginCanvas()`, directly or
	 through `Screen::begin(title)` or a standard view. Do not repeat the
	 underlying canvas setup in apps, OS apps, or watch faces. Delta-only updates
	 must preserve the existing framebuffer and use `presentDirty()` or
	 `present(bounds)` instead of clearing the canvas.
- `Input::wait()` consumes press events. Use its returned event rather than
	 polling again for the same press; hold-sensitive game controls must handle
	 held state through `Input::pressed(event)` without reading button GPIOs
	 directly. Route panel presentation through `Screen` rather than GxEPD2.
