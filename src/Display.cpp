...
namespace {
constexpr int16_t frameStride = WatchyDisplay::WIDTH / 8;
uint8_t lastFrame[frameStride * WatchyDisplay::HEIGHT];
bool lastFrameValid = false;
bool frameDiffActive = false;
WatchyUi::Bounds frameDiff{};

void invalidateFrameReference() {
  lastFrameValid = false;
  frameDiffActive = false;
}

#ifndef WATCHY_DETERMINISTIC_GALLERY
void compareFrame(const uint8_t *bitmap, int16_t x, int16_t y,
                  int16_t width, int16_t height) {
  int16_t left = x / 8;
  int16_t right = (x + width + 7) / 8;
  int16_t changedLeft = right;
  int16_t changedRight = left;
  int16_t changedTop = y + height;
  int16_t changedBottom = y;
  for (int16_t row = y; row < y + height; row++) {
    for (int16_t column = left; column < right; column++) {
      uint16_t offset = row * frameStride + column;
      if (!lastFrameValid || lastFrame[offset] != bitmap[offset]) {
        changedLeft = min(changedLeft, column);
        changedRight = max<int16_t>(changedRight, column + 1);
        changedTop = min(changedTop, row);
        changedBottom = row + 1;
      }
    }
    memcpy(lastFrame + row * frameStride + left,
           bitmap + row * frameStride + left, right - left);
  }
  frameDiffActive = true;
  frameDiff = changedRight > changedLeft
      ? WatchyUi::Bounds{static_cast<int16_t>(changedLeft * 8), changedTop,
                        static_cast<int16_t>((changedRight - changedLeft) * 8),
                        static_cast<int16_t>(changedBottom - changedTop)}
      : WatchyUi::Bounds{0, 0, 0, 0};
  if (x == 0 && y == 0 && width == WatchyDisplay::WIDTH &&
      height == WatchyDisplay::HEIGHT) lastFrameValid = true;
}
    #endif
}
...
void WatchyDisplay::initWatchy(bool forceFullRefresh) {
  if (forceFullRefresh) displayFullInit = true;
  ...
  hardwareInitialized = true;
  WatchyDiagnostics::recordDisplayInitialization();
}

void WatchyDisplay::ensureInitialized() {
  initWatchy();
}

bool WatchyDisplay::initializedThisWake() const {
  return hardwareInitialized;
}

bool WatchyDisplay::initialFullRefreshPending() const {
  return displayFullInit;
}
...
void WatchyDisplay::setDarkBorder(bool dark) {
  ...
  return;
#endif
  if (!hardwareInitialized || _hibernating) return;
  _startTransfer();
...
void WatchyDisplay::writeScreenBufferAgain(uint8_t value)
{
  ...
  ensureInitialized();
  if (!_using_partial_mode) _Init_Part();
  // If the controller still expects an initial full buffer write,
  // ensure both previous (0x26) and current (0x24) are initialized
  // so that partial differential updates compute correctly.
  if (_initial_write) {
...
{
  if (x == 0 && y == 0 && w == WIDTH && h == HEIGHT && !pgm) {
    WatchyDemo::ScreenCapture::emit(bitmap, WIDTH, HEIGHT);
  }
#ifndef WATCHY_DETERMINISTIC_GALLERY
  if (bitmap && x == 0 && y == 0 && w == WIDTH && h == HEIGHT &&
      !invert && !mirror_y && !pgm) {
    ensureInitialized();
    if (_initial_write) writeScreenBuffer();
    compareFrame(bitmap, x, y, w, h);
    if (frameDiff.width != 0) {
      _writeImagePart(0x24, bitmap, frameDiff.x, frameDiff.y, WIDTH, HEIGHT,
                     frameDiff.x, frameDiff.y, frameDiff.width, frameDiff.height,
                     false, false, false);
    }
    return;
  }
...
void WatchyDisplay::writeImageForFullRefresh(const uint8_t bitmap[], int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  if (bitmap && x == 0 && y == 0 && w == WIDTH && h == HEIGHT &&
      !invert && !mirror_y && !pgm) {
    memcpy(lastFrame, bitmap, sizeof(lastFrame));
    lastFrameValid = true;
  }
}
...
{
  if (w_bitmap == WIDTH && h_bitmap == HEIGHT && !pgm) {
    WatchyDemo::ScreenCapture::emit(bitmap, WIDTH, HEIGHT);
  }
#ifndef WATCHY_DETERMINISTIC_GALLERY
  if (bitmap && w_bitmap == WIDTH && h_bitmap == HEIGHT &&
      x_part == x && y_part == y && x >= 0 && y >= 0 && w > 0 && h > 0 &&
      x + w <= WIDTH && y + h <= HEIGHT && !invert && !mirror_y && !pgm) {
    ensureInitialized();
    if (_initial_write) writeScreenBuffer();
    compareFrame(bitmap, x, y, w, h);
    if (frameDiff.width != 0) {
...
{
  if (frameDiffActive) {
    writeImageAgain(bitmap, 0, 0, WIDTH, HEIGHT, false, false, false);
    return;
  }
  _writeImagePart(0x24, bitmap, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h, invert, mirror_y, pgm);
}
...
void WatchyDisplay::_Update_Full()
{
  ...
  WatchyDiagnostics::recordDisplayRefresh(true);
}
...
void WatchyDisplay::_Update_Part()
{
  ...
  WatchyDiagnostics::recordDisplayRefresh(false);
}
