...
void Watchy::showWhois() {
  ...
  while (true) {
    ...
    if (!NetAppCommon::pickTargetEditAndPersist(*this, "hist_whois", "Whois target", sHistorySelected, selectedTarget, sizeof(selectedTarget))) {
      showMenu(menuIndex);
      return;
    }
    ...
    if (!NetUtils::ensureWiFi(*this, 1, 10000)) {
      ...
      body = NetUtils::whoisLookup(host);
    }
