...
WatchyUi::Event eventFromWakeBits(uint64_t bits);
...
void ensureDisplayInitialized() {
  ...
  Serial.printf("@WATCHY_POWER ensureDisplayInitialized: before initWatchy initialized=%d\n", display.epd2.initializedThisWake() ? 1 : 0);
  #endif
  display.epd2.initWatchy(false);
  #ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER ensureDisplayInitialized: after initWatchy initialized=%d\n", display.epd2.initializedThisWake() ? 1 : 0);
  #endif
}
...
void showColdBootWatchFace() {
  ...
  #ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER showColdBootWatchFace: before asyncPowerOn initialized=%d\n", display.epd2.initializedThisWake() ? 1 : 0);
  #endif
  ...
  #ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.println("@WATCHY_POWER showColdBootWatchFace: after present");
  #endif
}
...
void beginBootRuntime() {
  ...
  Serial.flush();
#elif defined(WATCHY_POWER_DIAGNOSTICS)
  ...
  Serial.println("@WATCHY_POWER START");
#else
  setCpuFrequencyMhz(RUNTIME_CPU_FREQUENCY_MHZ);
#endif
}

void completeBootRuntime() {
...
void init(String datetime) {
  // 1) Capture wake context (cause + latched EXT1 bitmap) as early as possible.
  WakeContext wake = readWakeContext();

  // 2) Begin basic runtime services (CPU freq, serial for diagnostics if needed).
  beginBootRuntime();

  // 3) Initialize I2C and RTC
  initializeBusesAndRtc();

  // 4) Restore RTC-wakeup GPIOs to normal runtime configuration
  restoreRuntimeWakePins();

  // 5) Complete runtime initialization that depends on restored GPIOs
  completeBootRuntime();
  normalizeMenuStateAfterBoot();

  // 6) Initialize display (but avoid forcing full refresh here)
  // Do not initialize the display here; initialize lazily when a
  // wake handler actually needs to render to the panel.

  // 7) Dispatch wake handling using the captured WakeContext
  dispatchWake(wake, datetime);

  // 8) Finish and enter deep sleep path
  deepSleep();
}
...
void handleButtonWake(uint64_t wakeupBits) {
  ...
  if (heartRateWake && serviceWatchfaceHeartRateMonitoring() &&
    ...
    RTC.read(currentTime);
    synchronizeStepCounterDay(currentTime);
    // Lazily initialize the display only when we actually need to
    // perform a visible update (avoids powering the panel on for
    // unchanged FIFO wakes).
    ensureDisplayInitialized();
    display.epd2.asyncPowerOn();
    if (!refreshWatchFaceHeartRate()) {
      // Fall back to a partial full-face update when the active
      // watchface does not implement targeted HR refresh.
      showWatchFace(true);
    }
  }
  // Convert latched EXT1 button bits into a deterministic event and handle it.
  WatchyUi::Event event = eventFromWakeBits(wakeupBits & BTN_PIN_MASK);
...
void dispatchWake(const WakeContext &wake, String datetime) {
  ...
  case ESP_SLEEP_WAKEUP_EXT0: // RTC Alarm
    ...
    case MAIN_MENU_STATE:
      if (alreadyInMenu) {
        guiState = WATCHFACE_STATE;
        // Prefer a partial refresh when returning from the menu.
        showWatchFace(true);
      } else {
        alreadyInMenu = true;
      }
  ...
  case ESP_SLEEP_WAKEUP_EXT1: // button Press
    handleButtonWake(wake.ext1Bits);
    break;
#ifdef ARDUINO_ESP32S3_DEV
  case ESP_SLEEP_WAKEUP_EXT0: // USB plug in / change
...
void handleColdBoot(String datetime) {
  ...
  // Enable watchface heart-rate monitoring by default for all faces.
  // Fall detection or sensor availability may disable this internally.
  setWatchfaceHeartRateMonitoring(true);
...
void servicePendingWatchfaceWakeBeforeDeepSleep() {
  ...
  if (isWatchfaceHeartRateMonitoringActive() &&
      ...
      guiState == WATCHFACE_STATE) {
    RTC.read(currentTime);
    // Lazily initialize display and try a targeted HR-region refresh
    // before deep sleep. Fall back to full face refresh if needed.
    ensureDisplayInitialized();
    display.epd2.asyncPowerOn();
    if (!refreshWatchFaceHeartRate()) {
      showWatchFace(false);
...
bool configureRtcWakeInput(gpio_num_t pin) {
  esp_err_t r1 = rtc_gpio_init(pin);
  esp_err_t r2 = rtc_gpio_set_direction(pin, RTC_GPIO_MODE_INPUT_ONLY);
  esp_err_t r3 = rtc_gpio_pullup_en(pin);
#ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.begin(115200);
  Serial.printf("@WATCHY_POWER configureRtcWakeInput pin=%d rtc_gpio_init=%d set_dir=%d pullup=%d\n",
                static_cast<int>(pin), static_cast<int>(r1), static_cast<int>(r2), static_cast<int>(r3));
#endif
  return r1 == ESP_OK && r2 == ESP_OK && r3 == ESP_OK;
}

bool configureUsbWakeForDeepSleep() {
  bool plugged = WatchyUi::Power::usbPluggedIn();
  gpio_num_t pin = static_cast<gpio_num_t>(USB_DET_PIN);
  if (!configureRtcWakeInput(pin)) {
#ifdef WATCHY_POWER_DIAGNOSTICS
    Serial.printf("@WATCHY_POWER configureUsbWakeForDeepSleep: configureRtcWakeInput failed for pin=%d\n", static_cast<int>(pin));
#endif
    return false;
  }
  int wakeLevel = plugged ? LOW : HIGH; // wake when level changes to opposite of current
  if (rtc_gpio_get_level(pin) == wakeLevel) return false;
  esp_err_t r = esp_sleep_enable_ext0_wakeup(pin, wakeLevel);
#ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER configureUsbWakeForDeepSleep: pin=%d plugged=%d wakeLevel=%d esp_sleep_enable_ext0_wakeup=%d\n",
                static_cast<int>(pin), plugged ? 1 : 0, wakeLevel, static_cast<int>(r));
#endif
  return r == ESP_OK;
}
...
bool configureButtonWakeForDeepSleep() {
  ...
  for (gpio_num_t pin : buttonPins) {
    if (!configureRtcWakeInput(pin)) {
#ifdef WATCHY_POWER_DIAGNOSTICS
      Serial.printf("@WATCHY_POWER configureButtonWakeForDeepSleep: configureRtcWakeInput failed for button pin=%d\n", static_cast<int>(pin));
#endif
      return false;
    }
  }
  if (sensorWakeMask & ACC_INT_MASK &&
    ...
    #ifdef WATCHY_POWER_DIAGNOSTICS
    Serial.printf("@WATCHY_POWER configureButtonWakeForDeepSleep: configureRtcWakeInput failed for ACC_INT_1_PIN=%d\n", ACC_INT_1_PIN);
    #endif
    return false;
  }
  if (sensorWakeMask & ACC_INT_2_MASK &&
    ...
    #ifdef WATCHY_POWER_DIAGNOSTICS
    Serial.printf("@WATCHY_POWER configureButtonWakeForDeepSleep: configureRtcWakeInput failed for ACC_INT_2_PIN=%d\n", ACC_INT_2_PIN);
    #endif
    return false;
  }
  for (uint8_t pin = 0; pin < 22; pin++) {
    if ((wakeMask & BIT64(pin)) != 0 &&
        rtc_gpio_get_level(static_cast<gpio_num_t>(pin)) == ACTIVE_LOW) {
      return false;
    }
  }
  esp_err_t r = esp_sleep_enable_ext1_wakeup(wakeMask, ESP_EXT1_WAKEUP_ANY_LOW);
#ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER configureButtonWakeForDeepSleep: wakeMask=0x%016llx esp_sleep_enable_ext1_wakeup=%d\n",
                static_cast<unsigned long long>(wakeMask), static_cast<int>(r));
#endif
  return r == ESP_OK;
}
...
bool configureMinuteTimerWakeForDeepSleep() {
  ...
  esp_err_t r = esp_sleep_enable_timer_wakeup(
             static_cast<uint64_t>(secondsToNextWake) * uS_TO_S_FACTOR);
#ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER configureMinuteTimerWakeForDeepSleep: next=%u sec esp_sleep_enable_timer_wakeup=%d\n",
                static_cast<unsigned int>(secondsToNextWake), static_cast<int>(r));
#endif
  return r == ESP_OK;
}
...
bool configureWakeSourcesForDeepSleep() {
  esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_ALL);
  bool okUsb = configureUsbWakeForDeepSleep();
  bool okButtons = configureButtonWakeForDeepSleep();
  bool okTimer = configureMinuteTimerWakeForDeepSleep();
  if (!okUsb || !okButtons || !okTimer) {
#ifdef WATCHY_POWER_DIAGNOSTICS
    Serial.printf("@WATCHY_POWER configureWakeSourcesForDeepSleep: usb=%d buttons=%d timer=%d\n",
                  okUsb ? 1 : 0, okButtons ? 1 : 0, okTimer ? 1 : 0);
#endif
    esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_ALL);
    return false;
  }
...
void serviceUsbRuntime() {
  while (WatchyUi::Power::usbPluggedIn()) {
    uint8_t previousMinute = currentTime.Minute;
...
void deepSleep() {
  for (;;) {
    ...
    serviceUsbRuntime();
#endif
    stopRuntimeForDeepSleep();
    servicePendingWatchfaceWakeBeforeDeepSleep();
    RTC.clearAlarm();
    hibernateDisplayForDeepSleep();
#ifdef ARDUINO_ESP32S3_DEV
    if (configureWakeSourcesForDeepSleep()) {
      startDeepSleep();
    }
    esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_ALL);
    restoreRuntimeWakePins();
    WatchyUi::Event event = WatchyUi::Input::waitScheduled(100);
    if (event != WatchyUi::Event::NONE) handleButtonEvent(event);
    vTaskDelay(pdMS_TO_TICKS(20));
#else
    configureWakeSourcesForDeepSleep();
    startDeepSleep();
#endif
  }
}
...
WatchyUi::Event eventFromWakeBits(uint64_t bits) {
  if (bits & MENU_BTN_MASK) return WatchyUi::Event::MENU;
  if (bits & BACK_BTN_MASK) return WatchyUi::Event::BACK;
  if (bits & UP_BTN_MASK) return WatchyUi::Event::UP;
  if (bits & DOWN_BTN_MASK) return WatchyUi::Event::DOWN;
  return WatchyUi::Event::NONE;
}
...
bool syncNTP(long gmt, String ntpServer) {
  ...
  timeClient.begin();
  bool synced = timeClient.forceUpdate();
  if (synced) {
    tmElements_t tm;
