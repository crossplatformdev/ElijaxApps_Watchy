...
constexpr uint32_t buttonDebounceMs = 5;
...
uint8_t readButtons() {
  ...
  for (uint8_t index = 0; index < 4; index++) {
    if (digitalRead(buttonPins[index]) == ACTIVE_LOW) buttons |= 1U << index;
  }
  return buttons;
}

bool enableButtonWakeup() {
  for (gpio_num_t pin : buttonPins) gpio_wakeup_enable(pin, GPIO_INTR_LOW_LEVEL);
  if (auxiliaryWakeHandler != nullptr) {
    gpio_wakeup_enable(static_cast<gpio_num_t>(auxiliaryWakePin), GPIO_INTR_LOW_LEVEL);
  }
  gpio_wakeup_enable(static_cast<gpio_num_t>(USB_DET_PIN), GPIO_INTR_HIGH_LEVEL);
...
static void IRAM_ATTR notifyBackFromIsr(void *argument) __attribute__((unused));
static void IRAM_ATTR notifyBackFromIsr(void *argument) {
...
bool waitForButtonNotification(uint32_t durationMs) {
  ...
  if (readButtons() != previousButtons) {
...
void Canvas::centeredText(const Bounds &bounds, const char *text,
  ...
  // Mark the area we just drew so presentDirty can update only changed
  // regions.
  Screen::invalidate(Bounds{cursorX, cursorY,
                            static_cast<int16_t>(textWidth),
                            static_cast<int16_t>(textHeight)});
}

void Canvas::centeredText(const Bounds &bounds, const char *text,
                          uint8_t textSize, uint16_t color) {
  Watchy::display.setFont();
  uint8_t fittedSize = max<uint8_t>(1, textSize);
  int16_t textX;
  int16_t textY;
  uint16_t textWidth;
  uint16_t textHeight;
  do {
    Watchy::display.setTextSize(fittedSize);
    Watchy::display.getTextBounds(text == nullptr ? "" : text, 0, 0,
                                  &textX, &textY, &textWidth, &textHeight);
    if (fittedSize == 1 ||
        (textWidth <= bounds.width && textHeight <= bounds.height)) break;
    fittedSize--;
  } while (true);
  ...
  if (gray == GRAY_BLACK || gray == GRAY_WHITE) {
    Watchy::display.fillRect(left, top, right - left, bottom - top,
                            gray == GRAY_WHITE ? GxEPD_WHITE
                                               : GxEPD_BLACK);
    Screen::invalidate(Bounds{left, top, static_cast<int16_t>(right - left), static_cast<int16_t>(bottom - top)});
    return;
  }
...
void GrayPaint::line(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                     Gray8 gray) {
  int16_t sx = x0, sy = y0, ex = x1, ey = y1;
  ...
  int16_t left = min(sx, ex) - 1; if (left < 0) left = 0;
  int16_t top = min(sy, ey) - 1; if (top < 0) top = 0;
  int16_t right = max(sx, ex) + 1; if (right > DISPLAY_WIDTH) right = DISPLAY_WIDTH;
  int16_t bottom = max(sy, ey) + 1; if (bottom > DISPLAY_HEIGHT) bottom = DISPLAY_HEIGHT;
  Screen::invalidate(Bounds{left, top, static_cast<int16_t>(right - left), static_cast<int16_t>(bottom - top)});
}
...
void GrayPaint::bitmap8(int16_t x, int16_t y, uint16_t width,
  ...
  int16_t drawLeft = max<int16_t>(0, x);
  int16_t drawTop = max<int16_t>(0, y);
  int16_t drawWidth = static_cast<int16_t>(sourceRight - sourceLeft);
  int16_t drawHeight = static_cast<int16_t>(sourceBottom - sourceTop);
  Screen::invalidate(Bounds{drawLeft, drawTop, drawWidth, drawHeight});
}
...
void Screen::begin(const char *title) {
  ...
  uint16_t textWidth;
  uint16_t textHeight;
  Watchy::display.getTextBounds(title == nullptr ? "" : title, 0, 0,
                                &textX, &textY, &textWidth, &textHeight);
  if (textWidth > DISPLAY_WIDTH) {
    Canvas::centeredText({0, 0, DISPLAY_WIDTH, 20}, title, 1, Theme::foreground());
    return;
  }
...
void Screen::presentDirty(int nextGuiState) {
  ...
  for (uint8_t index = 0; index < dirtyRegionCount; index++) {
    ...
    // Perform a true windowed display update using the library's
    // windowed display function which transmits only the internal
    // framebuffer region and executes a localized partial refresh.
    Watchy::display.displayWindow(bounds.x, bounds.y,
...
void Widget::footer(const char *commands) {
  ...
  GrayPaint::line(Theme::screenMargin, 178,
                  ...
                  Theme::tone(ToneRole::Separator));
  constexpr uint8_t columns = (DISPLAY_WIDTH - 2 * Theme::screenMargin) / 6;
  if (strlen(commands) > columns) {
    paragraph(commands, Theme::screenMargin, 181, columns, 2, 9);
    return;
  }
...
void Power::idle(uint32_t durationMs) {
  ...
  if (readButtons() != 0) {
    vTaskDelay(max<TickType_t>(1, pdMS_TO_TICKS(buttonDebounceMs)));
    return;
  }
#ifdef ARDUINO_ESP32S3_DEV
  if (usbPluggedIn()) {
...
WakeupReason deepSleepDelay(uint32_t durationMs) {
  ...
  if (inputTask != nullptr && xTaskGetCurrentTaskHandle() == inputTask) {
    ...
    for (;;) {
      ...
      vTaskDelay(max<TickType_t>(1, pdMS_TO_TICKS(min<uint32_t>(remaining, 10))));
    }
...
Event Input::poll() {
  if (delayedBackPending) {
    delayedBackPending = false;
    if (millis() - delayedBackAt <= delayedBackTimeoutMs) {
      previousButtons = readButtons();
      return Event::BACK;
    }
  }
  uint8_t buttons = readButtons();
  if (buttons == previousButtons) return Event::NONE;
  vTaskDelay(max<TickType_t>(1, pdMS_TO_TICKS(buttonDebounceMs)));
  if (buttons != readButtons()) return Event::NONE;
  uint8_t pressed = buttons & ~previousButtons;
  previousButtons = buttons;
  if (pressed & 2U) return Event::BACK;
  if (pressed & 1U) return Event::MENU;
  if (pressed & 4U) return Event::UP;
  if (pressed & 8U) return Event::DOWN;
  return Event::NONE;
}

bool Input::pressed(Event event) {
  switch (event) {
  case Event::MENU: return digitalRead(MENU_BTN_PIN) == ACTIVE_LOW;
  case Event::BACK: return digitalRead(BACK_BTN_PIN) == ACTIVE_LOW;
  case Event::UP: return digitalRead(UP_BTN_PIN) == ACTIVE_LOW;
  case Event::DOWN: return digitalRead(DOWN_BTN_PIN) == ACTIVE_LOW;
...
void Input::waitForRelease(Event event) {
  uint8_t pin;
  switch (event) {
  case Event::BACK: pin = BACK_BTN_PIN; break;
  case Event::MENU: pin = MENU_BTN_PIN; break;
  case Event::UP: pin = UP_BTN_PIN; break;
  case Event::DOWN: pin = DOWN_BTN_PIN; break;
  default: return;
  }
  uint32_t startedAt = millis();
  while (digitalRead(pin) == ACTIVE_LOW &&
         millis() - startedAt < buttonReleaseTimeoutMs) {
    vTaskDelay(max<TickType_t>(1, pdMS_TO_TICKS(buttonDebounceMs)));
  }
  previousButtons = readButtons();
}
