...
public:
  ...
  void shutDown();
  void wakeUp();
  bool setAdvancedPowerSave(bool enabled);
  bool selfTest();
  ...
  bool configureAccelFifo(bool enabled, uint8_t downsampling = 0);
  bool setFifoWatermark(uint16_t bytes);
  ...
  bool mapInterrupt(uint8_t pinMap, uint16_t interrupt, bool enabled = true);
  bool configureAnyMotion(uint16_t threshold, uint16_t duration,
                          uint8_t axes = BMA423_ALL_AXIS_EN);
  bool getINT();
  uint16_t getIRQMASK();
