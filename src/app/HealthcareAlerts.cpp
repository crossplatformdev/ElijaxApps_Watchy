...
void check(const tmElements_t &now) {
  ensureLoaded();
  ...
  if (medicationEnabled && now.Hour == medicationHour &&
    ...
    Watchy::vibMotor(140, 12);
    EmergencyProfile::Data profile;
    EmergencyProfile::load(profile);
    WatchyHealthcareTools::showMedications(profile);
  }
  ...
  if (hydrationEnabled) {
    if (nextHydrationAt == 0) {
      ...
      Watchy::vibMotor(90, 6);
      nextHydrationAt = current + hydrationMinutes * SECS_PER_MIN;
      EmergencyProfile::Data profile;
      EmergencyProfile::load(profile);
      WatchyHealthcareTools::showHydration(profile);
    }
  } else {
    nextHydrationAt = 0;
  }
  ...
  if (checkInIsArmed && current >= checkInDueAt &&
    ...
    Watchy::vibMotor(150, 14);
    nextCheckInBuzzAt = current + 5 * SECS_PER_MIN;
    HealthcareAlerts::armCheckIn(nextCheckInBuzzAt);
  }
}
