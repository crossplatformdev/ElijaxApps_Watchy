...
void movePlayerPaddle(PongState &state, WatchyUi::Event queuedDirection,
                      bool upHeld, bool downHeld) {
  if (state.paused || state.gameOver) return;
  int direction = 0;
  if (queuedDirection == WatchyUi::Event::UP) direction = -1;
  else if (queuedDirection == WatchyUi::Event::DOWN) direction = 1;
  else if (upHeld != downHeld) direction = upHeld ? -1 : 1;
  state.playerY = clampPaddleY(state.playerY + direction * 12);
}
...
void showPongImpl(Watchy *watchy) {
  ...
  WatchyUi::Event queuedDirection = WatchyUi::Event::NONE;
  ...
  while (true) {
    ...
    if (event == WatchyUi::Event::MENU) {
      queuedDirection = WatchyUi::Event::NONE;
    ...
    if (!state.paused &&
        (event == WatchyUi::Event::UP || event == WatchyUi::Event::DOWN)) {
      queuedDirection = event;
    }

    if (!state.paused && millis() - lastFrame >= 120) {
      PongState previous = state;
      movePlayerPaddle(state, queuedDirection,
                       WatchyUi::Input::pressed(WatchyUi::Event::UP),
                       WatchyUi::Input::pressed(WatchyUi::Event::DOWN));
      queuedDirection = WatchyUi::Event::NONE;
