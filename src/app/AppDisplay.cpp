...
void drawStateDetail(const WatchyUi::Bounds &bounds, int16_t top,
                      const char *detail) {
  if (detail == nullptr || bounds.width <= 16) return;
  uint8_t columns = static_cast<uint8_t>((bounds.width - 16) / 6);
  uint8_t lines = static_cast<uint8_t>(max<int16_t>(0,
      min<int16_t>(bounds.y + bounds.height, 176) - top) / 10);
  if (strlen(detail) <= columns) {
    centered({bounds.x, top, bounds.width, 16}, detail);
  } else if (lines > 0) {
    WatchyUi::Widget::paragraph(detail, bounds.x + 8, top, columns, lines, 10);
  }
}
...
void drawEmptyState(const WatchyUi::Bounds &bounds, const char *label,
  ...
  drawStateDetail(bounds, bounds.y + iconSize + 35, detail);
}
