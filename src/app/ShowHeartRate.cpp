...
RTC_DATA_ATTR uint16_t lastValidHeartRateBpm = 0;
...
constexpr WatchyUi::Bounds appBpmBounds{76, 63, 112, 35};
constexpr WatchyUi::Bounds appStatusBounds{16, 116, 176, 24};
constexpr uint32_t watchfaceStateMagic = 0x4852424AUL;
...
WatchyBcg::Result processWatchfaceSample(const Accel &acceleration) {
  WatchyBcg::Result result = WatchyBcg::process(
      ...
      watchfaceSampleRateMilliHz, WatchyBcg::ResultMode::Live);
  if (result.windowComplete) {
    heartRateBpm = result.bpm;
    heartRateValid = result.valid;
    if (result.valid) lastValidHeartRateBpm = result.bpm;
  }
  return result;
}
...
bool runHeartRateMeasurement(HeartRateUpdateCallback updateCallback) {
  ...
  while (!stopRequested()) {
    ...
    nextSampleAt = now + samplePeriodMs;
    ...
    WatchyBcg::Result result = WatchyBcg::process(
      ...
      WatchyBcg::baselineSampleRateMilliHz, WatchyBcg::ResultMode::Live);
    if (result.windowComplete) {
      heartRateBpm = result.bpm;
      heartRateValid = result.valid;
      if (result.valid) lastValidHeartRateBpm = result.bpm;
...
void heartRateTask(void *parameter) {
  ...
  runHeartRateMeasurement(parameters.updateCallback);
...
bool measureHeartRateSilently(HeartRateUpdateCallback updateCallback) {
...
bool serviceWatchfaceHeartRateMonitoring() {
  ...
  uint16_t previousBpm = watchfaceHeartRateBpm();
  ...
  bool visibleChanged = previousBpm != watchfaceHeartRateBpm() ||
...
uint16_t watchfaceHeartRateBpm() {
  ...
  if (heartRateValid) return heartRateBpm;
#endif
  return lastValidHeartRateBpm;
}
...
void showHeartRateImpl(Watchy *watchy) {
  ...
  if (measureHeartRateSilently(queueAppHeartRateUpdate)) {
    ...
    beginAppDisplay("HEART RATE");
    AppVisual::drawWarningState({8, 38, 184, 118}, "SENSOR UNAVAILABLE",
                                "Measurement could not start");
    WatchyUi::Widget::footer("BACK EXIT");
    finishAppDisplay();
#ifndef WATCHY_BCG_TRACE_CAPTURE
...
void renderHeartRatePreview(uint8_t view) {
  ...
  Watchy::display.setTextSize(1);
  Watchy::display.setCursor(21, 51);
  Watchy::display.println(view == 3 ? "SIGNAL INVALID"
                                    : "EXPERIMENTAL BCG");
  WatchyUi::Widget::footer("BACK STOP");
