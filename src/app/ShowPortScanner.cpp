...
void showPortScannerImpl(Watchy *watchy) {
  String defaultTarget = NETWORKING_PORT_SCANNER_HOST;
  if (defaultTarget == "AUTO") {
    if (!NetworkApps::ensureConnected("PORT SCANNER", watchy)) {
      if (watchy != nullptr) watchy->showMenu(menuIndex, false);
      else WatchySdk::showMenu(menuIndex, false);
      return;
    }
    defaultTarget = WiFi.gatewayIP().toString();
    if (defaultTarget == "0.0.0.0") {
      defaultTarget = NETWORKING_PORT_SCANNER_FALLBACK_HOST;
    }
...
void Watchy::showPortScanner() { showPortScannerImpl(this); }

void WatchySdk::showPortScanner() { showPortScannerImpl(nullptr); }
