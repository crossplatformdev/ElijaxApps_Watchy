...
void drawOthello(const OthelloState &state) {
  ...
  char youText[8];
  snprintf(youText, sizeof(youText), "YOU %d", humanPieces);
  Watchy::display.setCursor(8, AppVisual::centeredCursorY(9, youText));
  Watchy::display.print(youText);
  char cpuText[8];
  snprintf(cpuText, sizeof(cpuText), "CPU %d", computerPieces);
  Watchy::display.setCursor(150, AppVisual::centeredCursorY(9, cpuText));
  Watchy::display.print(cpuText);
  const char *label = statusText(state.status);
  Watchy::display.setCursor((DISPLAY_WIDTH - strlen(label) * 6) / 2,
                            AppVisual::centeredCursorY(9, label));
...
void showOthelloImpl(Watchy *watchy) {
...
void Watchy::showOthello() { showOthelloImpl(this); }

void WatchySdk::showOthello() { showOthelloImpl(nullptr); }
...
void renderOthelloPreview(uint8_t view) {
  ...
  if (view == 0) {
    ...
    for (uint8_t cell = 0; cell < boardSize * boardSize; cell++) {
      if (view == 2) {
        state.board[cell] = cell < 42 ? HUMAN : COMPUTER;
      } else if (view == 3) {
        state.board[cell] = cell < 20 ? HUMAN : COMPUTER;
      } else {
        state.board[cell] = cell % 2 == 0 ? HUMAN : COMPUTER;
      }
    }
    state.status = view == 2 ? HUMAN_WINS : view == 3 ? AI_WINS : DRAW_GAME;
    state.gameOver = true;
  }
  drawOthello(state);
}
