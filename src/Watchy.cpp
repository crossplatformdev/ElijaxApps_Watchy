...
constexpr uint32_t LOW_POWER_CPU_FREQUENCY_MHZ = 8;
...
void sensorDelay(uint32_t durationMs) {
  ...
  Watchy32KRTC &Watchy::RTC = WatchySdk::Device::RTC;
#else
  WatchyRTC &Watchy::RTC = WatchySdk::Device::RTC;
#endif
GxEPD2_BW<WatchyDisplay, WatchyDisplay::HEIGHT> &Watchy::display =
    WatchySdk::Device::display;
...
void Watchy::ensureDisplayInitialized() {
  ...
  Serial.printf("@WATCHY_POWER Watchy::ensureDisplayInitialized: before initWatchy initialized=%d\n", display.epd2.initializedThisWake() ? 1 : 0);
  #endif
  display.epd2.initWatchy();
  #ifdef WATCHY_POWER_DIAGNOSTICS
  Serial.printf("@WATCHY_POWER Watchy::ensureDisplayInitialized: after initWatchy initialized=%d\n", display.epd2.initializedThisWake() ? 1 : 0);
  #endif
}
