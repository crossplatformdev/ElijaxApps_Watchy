...
bool BMA423::setAdvancedPowerSave(bool enabled) {
  return BMA4_OK == bma4_set_advance_power_save(
                        enabled ? BMA4_ENABLE : BMA4_DISABLE, &__devFptr);
}
...
bool BMA423::configureAccelFifo(bool enabled, uint8_t downsampling) {
  uint16_t result = bma4_set_fifo_config(BMA4_FIFO_HEADER, BMA4_DISABLE,
                                         &__devFptr);
  result |= bma4_set_fifo_down_accel(downsampling, &__devFptr);
  result |= bma4_set_accel_fifo_filter_data(BMA4_ENABLE, &__devFptr);
  result |= bma4_set_fifo_config(BMA4_FIFO_ACCEL,
                                 enabled ? BMA4_ENABLE : BMA4_DISABLE,
                                 &__devFptr);
  result |= bma4_set_command_register(0xB0, &__devFptr);
  return result == BMA4_OK;
}

bool BMA423::setFifoWatermark(uint16_t bytes) {
  return bma4_set_fifo_wm(bytes, &__devFptr) == BMA4_OK;
}

bool BMA423::readAccelFifo(Accel *samples, uint16_t capacity,
                           uint16_t &sampleCount) {
  constexpr uint16_t frameBytes = BMA4_ACCEL_DATA_LENGTH;
  constexpr uint16_t readBlockBytes = 120;
  uint8_t fifoData[readBlockBytes];
  uint16_t remainingBytes = 0;
  sampleCount = 0;

  if (samples == nullptr || capacity == 0 ||
      bma4_get_fifo_length(&remainingBytes, &__devFptr) != BMA4_OK) {
    return false;
  }

  while (remainingBytes >= frameBytes && sampleCount < capacity) {
    uint16_t blockBytes = min<uint16_t>(remainingBytes, readBlockBytes);
    blockBytes = min<uint16_t>(blockBytes,
                               (capacity - sampleCount) * frameBytes);
    blockBytes -= blockBytes % frameBytes;
    if (blockBytes == 0) {
      break;
    }
    ...
    bma4_fifo_frame *previousFrame = __devFptr.fifo;
    __devFptr.fifo = &frame;
    uint16_t result = bma4_read_fifo_data(&__devFptr);
    uint16_t blockSamples = blockBytes / frameBytes;
    if (result == BMA4_OK) {
      result = bma4_extract_accel(samples + sampleCount, &blockSamples,
                                  &__devFptr);
    }
    __devFptr.fifo = previousFrame;
    if (result != BMA4_OK) {
      return false;
    }
...
bool BMA423::setInterruptMode(uint8_t mode) {
  return BMA4_OK == bma4_set_interrupt_mode(mode, &__devFptr);
}

bool BMA423::mapInterrupt(uint8_t pinMap, uint16_t interrupt, bool enabled) {
  return BMA4_OK == bma423_map_interrupt(
                        pinMap, interrupt,
                        enabled ? BMA4_ENABLE : BMA4_DISABLE, &__devFptr);
}

bool BMA423::configureAnyMotion(uint16_t threshold, uint16_t duration,
                                uint8_t axes) {
  bma423_anymotion_config configuration{};
  configuration.threshold = threshold;
  configuration.duration = duration;
  configuration.nomotion_sel = 0;
  return BMA4_OK == bma423_set_any_motion_config(&configuration, &__devFptr) &&
         BMA4_OK == bma423_anymotion_enable_axis(axes, &__devFptr);
}
...
uint16_t BMA423::getIRQMASK() { return __IRQ_MASK; }
