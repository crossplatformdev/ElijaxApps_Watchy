...
void WatchyDisplay::busyCallback(const void *) {
  ...
  // ESP32-S3 uses native USB. Entering light sleep while awake can suspend USB
  // and make the host see a disconnect (common during heavy e-paper refresh).
  // Avoid sleep here; just yield briefly while waiting for the panel.
  yield();
  delay(1);
  return;
#endif
...
{
  // Prepare both current and previous buffers with a uniform background
  // colour, but defer the actual panel refresh to the caller. This lets
  // higher-level code follow the pattern:
  //   - set window
  //   - clearScreen(background)
  //   - draw all elements
  //   - single display.display(partial/full) at the end.
