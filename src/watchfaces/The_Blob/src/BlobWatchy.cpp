...
static Vector EDGE_VECTORS[VECTOR_SIZE];
...
void BlobWatchy::drawWatchFace()
{
  ...
  for (int i = 0; i < VECTOR_SIZE; i += STEP_HOUR)
  {
    ...
    UiSDK::drawLine(display, v1.x + 1, v1.y, v2.x + 1, v2.y, fgColor_);
    UiSDK::drawLine(display, v1.x - 1, v1.y, v2.x - 1, v2.y, fgColor_);
    UiSDK::drawLine(display, v1.x, v1.y + 1, v2.x, v2.y + 1, fgColor_);
    UiSDK::drawLine(display, v1.x, v1.y - 1, v2.x, v2.y - 1, fgColor_);
  }

  UiSDK::drawCircle(display, CENTER.x, CENTER.y, BLOB_RADIUS * BATTERY_WARNING, fgColor_);
