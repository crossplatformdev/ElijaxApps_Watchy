...
void DaliWatchy::drawWatchFace()
{
  ...
  for (int i = 0; i < VECTOR_SIZE; i++)
  {
    ...
    UiSDK::drawLine(display, v1.x, v1.y, v2.x, v2.y, fgColor);
    UiSDK::drawLine(display, v4.x, v4.y, v6.x, v6.y, fgColor);
  }
...
void DaliWatchy::drawLine(int x, int y, int w, VectorInt v0, Vector uv0, VectorInt a, Vector uv1, VectorInt b, Vector uv2, double invDen, const uint8_t *bitmap, int16_t bw, int16_t bh)
{
  for (int i = 0; i < w; i++)
  {
    ...
    UiSDK::drawPixel(display, x + i, y, white ? bgColor : fgColor);
  }
}
...
void showWatchFace_Dali(Watchy &watchy) {
  DaliWatchy face(watchy.settings);
