...
RTC_DATA_ATTR int worldTimeIntervalCounterTetris = 0;

void WatchySyncedTetris::readWorldTime()
{
    if (worldTimeIntervalCounterTetris == 0)
    {
        worldTimeIntervalCounterTetris = WTA_UPDATE_SHORT_INTERVAL;
        if (connectWiFi())
        {
            HTTPClient http;
            ...
            if (httpResponseCode == 200)
            {
                ...
                worldTimeIntervalCounterTetris = WTA_UPDATE_LONG_INTERVAL;
            }
            http.end();
            WiFi.mode(WIFI_OFF);
            btStop();
    ...
    {
        worldTimeIntervalCounterTetris = worldTimeIntervalCounterTetris < 0 ? 0 : worldTimeIntervalCounterTetris - 1;
    }
}
