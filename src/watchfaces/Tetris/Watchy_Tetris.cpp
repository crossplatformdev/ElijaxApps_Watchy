...
void WatchyTetris::drawWatchFace()
{
    ...
    // Background
    UiSDK::drawBitmap(display, 0, 0, tetrisbg, DISPLAY_WIDTH, DISPLAY_HEIGHT, fgColor_);

    //Hour
    UiSDK::drawBitmap(display, 25, 20, tetris_nums_0[currentTime.Hour / 10], 40, 60, fgColor_); //first digit
    UiSDK::drawBitmap(display, 75, 20, tetris_nums_1[currentTime.Hour % 10], 40, 60, fgColor_); //second digit

    //Minute
    UiSDK::drawBitmap(display, 25, 110, tetris_nums_2[currentTime.Minute / 10], 40, 60, fgColor_); //first digit
    UiSDK::drawBitmap(display, 75, 110, tetris_nums_3[currentTime.Minute % 10], 40, 60, fgColor_); //second digit
    ...
    //Random piece
    UiSDK::drawBitmap(display, 150, 140, pieces[int(random() * 19)], 40, 40, fgColor_);
}
...
void WatchyTetris::drawNumber(int x, int y, int value, int max_digits)
{
    ...
    {
        UiSDK::drawBitmap(display, x - i * 10, y, tetris_small_nums[value % 10], 8, 8, fgColor_);
        value /= 10;
        if (value == 0)
            break;
    }
...
void showWatchFace_Tetris(Watchy &watchy)
{
    WatchyTetris face(watchy.settings);
