    ...
    for (int x = 0; x < 200; ++x)
    {
      ...
      { 
        if (totalDistance > EXTRA_RADIUS)
        {
          ...
          normal += CENTER;
          const bool isInk = getColor3(x, y, normal.x, normal.y, MatCapSource, 200, 200);
          UiSDK::drawPixel(display, x, y, isInk ? fgColor : bgColor);
        }
        else if (totalDistance >= EXTRA_RADIUS - 1.0f)
          UiSDK::drawPixel(display, x, y, fgColor);
      }
    }
  }
}

void showWatchFace_MetaBall(Watchy &watchy) {
  MetaBallWatchy face(watchy.settings);
