...
class QLock : public Watchy {
   public:
    ...
    bgColor = UiSDK::getWatchfaceBg(BASE_POLARITY);
    fgColor = UiSDK::getWatchfaceFg(BASE_POLARITY);

    UiSDK::drawBitmap(display, 0, 0, epd_bitmap_qlock, 200, 200, fgColor, bgColor);
   ...
   private:
    ...
    void setMinutes() {
        ...
        switch (m % 5) {
            case 0:
                UiSDK::drawBitmap(display, 0, 0, epd_bitmap_chequerboard, 16, 18,
                                   fgColor);
            case 1:
                UiSDK::drawBitmap(display, 200 - 11, 0, epd_bitmap_chequerboard, 16, 18,
                                   fgColor);
            case 2:
                UiSDK::drawBitmap(display, 200 - 12, 200 - 10, epd_bitmap_chequerboard,
                                   16, 18, fgColor);
            case 3:
                UiSDK::drawBitmap(display, 0, 200 - 10, epd_bitmap_chequerboard, 16, 18,
                                   fgColor);
            case 4:
                break;
        }
    ...
    void draw() {
        ...
        for (size_t y = 0; y < 10; y++) {
            ...
            while (bitset != 0) {
                if (bitset & 0x1) {
                    UiSDK::drawBitmap(display, 12 + (10 - x) * 16, 10 + y * 18,
...
void WatchfaceEntrypoint_QLock(Watchy &watchy) {
    QLock face;
...
void showWatchFace_QLock(Watchy &watchy) {
    WatchfaceEntrypoint_QLock(watchy);
}
