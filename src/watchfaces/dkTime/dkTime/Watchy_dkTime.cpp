...
void WatchyDkTime::drawWatchFace(){
  ...
  const uint16_t fgColor = UiSDK::getWatchfaceFg(BASE_POLARITY);
    ...
    UiSDK::drawBitmap(display, 0, 0, bg, DISPLAY_WIDTH, DISPLAY_HEIGHT, fgColor);

    //Hour
    UiSDK::drawBitmap(display, 35, 33, dk_nums[Hour/10], 28, 26, fgColor); //first digit
    UiSDK::drawBitmap(display, 66, 33, dk_nums[Hour%10], 28, 26, fgColor); //second digit

    //Minute
    UiSDK::drawBitmap(display, 105, 33, dk_nums[currentTime.Minute/10], 28, 26, fgColor); //first digit
    UiSDK::drawBitmap(display, 137, 33, dk_nums[currentTime.Minute%10], 28, 26, fgColor); //second digit

    if(dktime_playAnim) {
      drawAnim();
      dktime_playAnim = false;
    } else {
      //Static DK
      UiSDK::fillRect(display, 0, 73, 200, 124, bgColor);
      UiSDK::drawBitmap(display, 7, 76, dk_anim1[0], 182, 121, fgColor);
     ...
     //Choose an Animation
      ...
      if (randNum == 0){
        for(uint8_t i=0; i<animOneFrames; i++){
          UiSDK::drawBitmap(display, 7, 76, dk_anim1[i % animFrames], 182, 121, fgColor);
          UiSDK::displayUpdate(display, true);
          UiSDK::fillRect(display, 0, 76, 200, 121, bgColor);
        }
      } else if (randNum == 1) {
        for(uint8_t i=0; i<animTwoFrames; i++){
          UiSDK::drawBitmap(display, 7, 76, dk_anim2[i % animFrames], 182, 121, fgColor);
          UiSDK::displayUpdate(display, true);
          UiSDK::fillRect(display, 0, 76, 200, 121, bgColor);
        }
      } else {
        for(uint8_t i=0; i<animThreeFrames; i++){
          UiSDK::drawBitmap(display, 0, 76, dk_anim3[i % animThreeFrames], 200, 121, fgColor);
          UiSDK::displayUpdate(display, true);
          UiSDK::fillRect(display, 0, 76, 200, 121, bgColor);
        }
      }
