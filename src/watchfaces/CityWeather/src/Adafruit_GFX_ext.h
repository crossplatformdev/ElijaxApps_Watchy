...
static inline uint16_t themeFg() {
  return UiSDK::getWatchfaceFg(BASE_POLARITY);
}

static inline uint16_t themeBg() {
  return UiSDK::getWatchfaceBg(BASE_POLARITY);
}

void drawOutlinedBitmap(Adafruit_GFX &d, int16_t x, int16_t y, const unsigned char* bitmap , int16_t w, int16_t h, uint16_t color) {
  uint16_t borderColor = (color == themeFg()) ? themeBg() : themeFg();
...
void drawOutlinedText(Adafruit_GFX &d, int16_t x, int16_t y, const String &text, uint16_t textColor) {

  uint16_t borderColor = (textColor == themeFg()) ? themeBg() : themeFg();
  d.setTextColor(borderColor);
  for (int8_t dx = -2; dx <= 2; dx++) {
    for (int8_t dy = -2; dy <= 2; dy++) {
      if (dx != 0 || dy != 0) {
...
void printCentered(Adafruit_GFX &d, const String &text, int16_t centerX, int16_t y, uint16_t textColor = themeFg()) { 
  int16_t x1, y1;
  uint16_t w, h;
  d.getTextBounds(text, 0, 0, &x1, &y1, &w, &h);
  int16_t x = centerX - w / 2 - x1;
  drawOutlinedText(d, x, y, text, textColor);
}
...
void drawLine(Adafruit_GFX &d, int x0, int y0, int x1, int y1, uint16_t c = themeFg(), int dithering = 2) {
...
void fillRect(Adafruit_GFX &d,  int x, int y, int w, int h, uint16_t c = themeFg(), int dithering = 2) {
