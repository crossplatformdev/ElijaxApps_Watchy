...
static Vector EDGE_NORMAL[VECTOR_SIZE];
...
void SpiralWatchy::drawWatchFace()
{
  ...
  for (int i = minute; i < VECTOR_SIZE * 3 + minute; i++)
  {
    ...
    UiSDK::drawLine(display, v1.x, v1.y, v2.x, v2.y, fgColor_);
    UiSDK::drawLine(display, v4.x, v4.y, v6.x, v6.y, fgColor_);
  }
  ...
  for (int i = VECTOR_SIZE * 3 + minute; i < VECTOR_SIZE * 4 + minute - 1; i++)
  {
    ...
    UiSDK::drawTriangle(display, v1.x, v1.y, v4.x, v4.y, v2.x, v2.y, fgColor_);
    UiSDK::drawTriangle(display, v4.x, v4.y, v2.x, v2.y, v6.x, v6.y, fgColor_);
  }

  fillTriangle(SHADOW_CORNER_1, SHADOW_CORNER_1, SHADOR_CORNER_2, SHADOR_CORNER_2, SHADOR_CORNER_3, SHADOR_CORNER_3, SpiralFaceShadowCenter, 200, 200, fgColor_);
  fillTriangle(SHADOR_CORNER_3, SHADOR_CORNER_3, SHADOR_CORNER_4, SHADOR_CORNER_4, SHADOW_CORNER_1, SHADOW_CORNER_1, SpiralFaceShadowCenter, 200, 200, fgColor_);
...
void SpiralWatchy::DrawHand(float angle, float size)
{
  ...
  for (int i = 0; i < HAND_OUTLINE_LEN; i++)
  {
    ...
    UiSDK::drawLine(display, currentPoint.x, currentPoint.y, nextPoint.x, nextPoint.y, fgColor_);
...
void SpiralWatchy::drawLine(int x, int y, int w, VectorInt v0, Vector uv0, VectorInt a, Vector uv1, VectorInt b, Vector uv2, float invDen, const uint8_t *bitmap, int16_t bw, int16_t bh)
{
  for (int i = 0; i < w; i++)
  {
    ...
    UiSDK::drawPixel(display, x + i, y, white ? bgColor_ : fgColor_);
  }
}
...
void showWatchFace_Spiral_Watchy(Watchy &watchy) {
  SpiralWatchy face(watchy.settings);
