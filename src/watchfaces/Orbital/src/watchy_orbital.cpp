...
void WatchyOrbital::drawWatchFace() {
    ...
    sun.setPosition((double)lat, (double)lon, (int)(settings.gmtOffset / 3600));
    sun.setCurrentDate(tmYearToCalendar(currentTime.Year), currentTime.Month, currentTime.Day);
    UiSDK::setTextColor(display, secondaryColor);
    drawMoon();
    drawWatchTime();
    drawBattery();
    drawDayTime();
...
void WatchyOrbital::drawWatchTime() {
    ...
    UiSDK::setCursor(display, 0, 13);
    UiSDK::print(display, dayStr(currentTime.Wday));

    /*UiSDK::setCursor(display, 159, 195);
    UiSDK::println(display, tmYearToCalendar(currentTime.Year));*/
}
...
void WatchyOrbital::drawMoon() {
    ...
    UiSDK::setFont(display, &MoonPhases20pt7b);
    UiSDK::setCursor(display, moonX, moonY);
    UiSDK::print(display, (char)(moonphase+0x50));
}
...
void WatchyOrbital::drawWeather(){
    ...
    if(currentWeather.external){
        ...
        UiSDK::setFont(display, &OpenWeather_Regular12pt7b);
        int16_t x1;
        int16_t y1;
        uint16_t width;
        uint16_t height;
        UiSDK::getTextBounds(display, String(icon), 0, 0, &x1, &y1, &width, &height);
        UiSDK::setCursor(display, 200-36, 27);
        UiSDK::print(display, icon);
        
        UiSDK::setFont(display, &FreeSansBold9pt7b);
        UiSDK::setCursor(display, 165, 180);
        UiSDK::print(display, String(currentWeather.temperature)+((settings.weatherUnit == "metric")?" C":" F"));
    }
    UiSDK::setFont(display, &FreeSansBold9pt7b);
    UiSDK::setCursor(display, 165, 195);
    UiSDK::print(display, sensor.readTemperature());
}
...
void WatchyOrbital::fillArc2(float start_angle, float end_angle, unsigned int radius, unsigned int width, unsigned int colour, float step) {
    ...
    for (float i = start_angle; i <= end_angle; i += step) {
        ...
        UiSDK::drawLine(display, cx1, cy1, cx2, cy2, colour);
    }
}
...
void WatchyOrbital::fillArc3(float start_angle, float end_angle, unsigned int radius, unsigned int width, unsigned int colour, float step) {
    ...
    for (float i = start_angle; i <= end_angle - step; i += step) {
        ...
        UiSDK::fillTriangle(display, cx1, cy1, cx2, cy2, cx3, cy3, colour);
        UiSDK::fillTriangle(display, cx2, cy2, cx3, cy3, cx4, cy4, colour);
    }
}
...
void WatchyOrbital::drawEllipse(int16_t x0, int16_t y0, int16_t rx, int16_t ry, uint16_t color) {
    ...
    UiSDK::writePixel(display, x0 + x, y0 + y, color);
    UiSDK::writePixel(display, x0 - x, y0 + y, color);
    UiSDK::writePixel(display, x0 + x, y0 - y, color);
    UiSDK::writePixel(display, x0 - x, y0 - y, color);
    //region1
...
void WatchyOrbital::fillEllipse(int16_t x0, int16_t y0, int16_t rx, int16_t ry, uint16_t color) {
    ...
    UiSDK::writeFastVLine(display, x0, y0 - y, 2 * y, color);
    ...
    while (px < py) {
        ...
        UiSDK::writeFastVLine(display, x0 + x, y0 - y, 2 * y, color);
        UiSDK::writeFastVLine(display, x0 - x, y0 - y, 2 * y, color);
    }
...
void WatchyOrbital::drawDetails(float angle, unsigned int radius, String value){
    ...
    uint16_t width;
    uint16_t height;
    UiSDK::getTextBounds(display, value, 0, 0, &x1, &y1, &width, &height);*/
...
void showWatchFace_Orbital(Watchy &watchy) {
    WatchyOrbital face(watchy.settings);
