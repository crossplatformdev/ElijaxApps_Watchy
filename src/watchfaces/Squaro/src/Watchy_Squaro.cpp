...
void Squaro_Watchface::drawWatchFace(){
    ...
    primaryColor = UiSDK::getWatchfaceFg(BASE_POLARITY);
    secondaryColor = UiSDK::getWatchfaceBg(BASE_POLARITY);

    UiSDK::drawBitmap(display, 2, 2, squaro_allArray[currentTime.Hour/10], 95, 95, primaryColor);
    UiSDK::drawBitmap(display, 102, 2, squaro_allArray[currentTime.Hour%10], 95, 95, primaryColor);
    UiSDK::drawBitmap(display, 2, 102, squaro_allArray[currentTime.Minute/10], 95, 95, primaryColor);
    UiSDK::drawBitmap(display, 102, 102, squaro_allArray[currentTime.Minute%10], 95, 95, primaryColor);
}
...
void showWatchFace_Squaro(Watchy &watchy) {
    Squaro_Watchface face(watchy.settings);
