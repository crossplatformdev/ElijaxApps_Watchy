...
class StarryHorizon : public Watchy {
    public:
        ...
        void drawWatchFace(){
          ...
          drawGrid(bgColor);
          drawStars(STARS, fgColor);
          drawTime(fgColor);
          drawDate(fgColor, bgColor);
        }
        void drawGrid(uint16_t gridColor) {
          int prevY = horizonY;
        ...
        void drawStars(const Star stars[], uint16_t starColor) {
          // draw field of stars
...
//          printf("Minute %d, angle %f\n", (int)currentTime.Minute, minuteAngle);
        ...
        void drawDate(uint16_t fgColor, uint16_t bgColor) {
...
//          printf("bounds: %d x %d y, %d x1 %d y1, %d w, %d h\n", 0, 100, x1, y1, w, h);
          ...
          if(drawBg) {
            int padY = 3;
            int padX = 10;
            UiSDK::fillRect(display, x - (w / 2 + padX), y - (h + padY), w + padX*2, h + padY*2, UiSDK::getWatchfaceBg(BASE_POLARITY));
          }
          // uncomment to draw bounding box
//          UiSDK::drawRect(display, x - w / 2, y - h, w, h, GxEPD_WHITE);
