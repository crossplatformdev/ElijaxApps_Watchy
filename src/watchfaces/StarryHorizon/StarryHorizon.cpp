...
#include "WatchySdk.h" //include the Watchy library
...
class StarryHorizon : public Watchy {
    public:
        ...
        void drawWatchFace(){
          display.fillCircle(100, horizonY + planetR, planetR, WatchyUi::Theme::foreground());
          drawGrid();
          drawStars(STARS);
          drawTime();
          drawDate();
          // Mark the full screen dirty for the initial migration batch.
...
//          printf("Minute %d, angle %f\n", (int)WatchySdk::currentTime.Minute, minuteAngle);
          ...
          for(int starI = 0; starI < STAR_COUNT; starI++) {
            ...
            if(starR == 0) {
              display.drawPixel(rotated.x, rotated.y, WatchyUi::Theme::foreground());
            } else {
              display.fillCircle(rotated.x, rotated.y, starR, WatchyUi::Theme::foreground());
            }
          }
        }
        void drawTime() {
          ...
          char* timeStr;
          asprintf(&timeStr, "%d:%02d", WatchySdk::currentTime.Hour, WatchySdk::currentTime.Minute);
          drawCenteredString(timeStr, 100, 115, false);
          free(timeStr);
        }

        void drawDate() {
          String monthStr = monthShortStr(WatchySdk::currentTime.Month);
          String dayOfWeek = dayShortStr(WatchySdk::currentTime.Wday);
          display.setFont(&FreeSansBold9pt7b);
          display.setTextColor(WatchyUi::Theme::foreground());
          display.setTextWrap(false);
          char* dateStr;
          asprintf(&dateStr, "%s %s %d", dayOfWeek.c_str(), monthStr.c_str(), WatchySdk::currentTime.Day);
...
//          printf("bounds: %d x %d y, %d x1 %d y1, %d w, %d h\n", 0, 100, x1, y1, w, h);
          display.setCursor(x - w / 2, y);
            ...
            int padY = 3;
            int padX = 10;
            display.fillRect(x - (w / 2 + padX), y - (h + padY), w + padX*2, h + padY*2, WatchyUi::Theme::background());
          }
