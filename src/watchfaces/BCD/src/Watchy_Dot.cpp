...
void WatchyDot::printCentered(uint16_t x, uint16_t y, String text, uint16_t size=1){
    ...
    for(int i=-size-3;i<size+4;i++){
        for(int j=-size-3;j<size+4;j++){
            UiSDK::setCursor(display, x-w/2+i, y+h/2+j);
...
void WatchyDot::drawTime(){
    ...
    //printCentered(x, y, "h");
    UiSDK::fillCircle(display, x, y, 16, FOREGROUND_COLOR);
    UiSDK::fillCircle(display, x, y, 12, BACKGROUND_COLOR);
    ...
    UiSDK::fillCircle(display, x, y, 12, BACKGROUND_COLOR);
    UiSDK::fillCircle(display, x, y, 8, FOREGROUND_COLOR);
}
...
void WatchyDot::drawAlarm(){
    ...
    printCentered(100, 153, String(bcd_alarm_timer) + " min.");
}
