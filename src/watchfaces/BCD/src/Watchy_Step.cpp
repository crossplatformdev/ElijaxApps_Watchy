...
void WatchyStep::drawWatchFace(){
    ...
    for(uint8_t i=0; i<4; i++){
        UiSDK::drawRoundRect(display, 0+i, 0+i, 196, 196, 4, FOREGROUND_COLOR);
    }
...
void WatchyStep::drawTime(){
    UiSDK::setFont(display, &FONT_LARGE_BOLD);
    UiSDK::setCursor(display, 12, 42);
...
void WatchyStep::drawBattery(){
    ...
    UiSDK::drawBitmap(display, 145, 15, battery, 37, 21, FOREGROUND_COLOR);
    UiSDK::setFont(display, &FONT_SMALL_BOLD);
    UiSDK::setCursor(display, 152, 31);
    if(percentage < 10) {
        UiSDK::print(display, "0");
    }
...
void WatchyStep::drawSteps(){
    ...
    // Bottom line
    UiSDK::drawLine(display, 5, 140, 195, 140, FOREGROUND_COLOR);

    // Text of bottom line
    UiSDK::setFont(display, &FONT_SMALL);
    int16_t  x1, y1;
    uint16_t width, height;
    UiSDK::getTextBounds(display, "12", 55, 195, &x1, &y1, &width, &height);
    UiSDK::setCursor(display, 4+12*8 + 2-int(width/2), 160);
    UiSDK::println(display, "12");
    UiSDK::setCursor(display, 4+6*8 + 2-int(width/2), 160);
    UiSDK::println(display, "06");
    UiSDK::setCursor(display, 4+18*8 + 2-int(width/2), 160);
    UiSDK::println(display, "18");
    ...
    UiSDK::setCursor(display, 20, 140-max_height-5);
    UiSDK::print(display, max_steps);
    ...
    for(int h=0; h < 24; h++){
        ...
        UiSDK::drawLine(display, 4+h*8 + 2, 140, 4+h*8 + 2, 140-relative_steps, GRAPH_COLOR);
        UiSDK::drawLine(display, 4+h*8 + 3, 140, 4+h*8 + 3, 140-relative_steps, GRAPH_COLOR);
        UiSDK::drawLine(display, 4+h*8 + 4, 140, 4+h*8 + 4, 140-relative_steps, GRAPH_COLOR);
        UiSDK::drawLine(display, 4+h*8 + 5, 140, 4+h*8 + 5, 140-relative_steps, GRAPH_COLOR);
        ...
        if(steps_hours_yesterday[h] > 0){
            ...
            UiSDK::drawLine(display, 4+h*8 + 2, 140-relative_steps, 4+h*8 + 5, 140-relative_steps, color);
            UiSDK::drawLine(display, 4+h*8 + 2, 140-relative_steps-1, 4+h*8 + 5, 140-relative_steps-1, color);
        }
    }

    // Small indicator for current position
    UiSDK::drawLine(display, 4+currentTime.Hour*8 + 3, 140, 4+currentTime.Hour*8 + 3, 146, FOREGROUND_COLOR);
    UiSDK::drawLine(display, 4+currentTime.Hour*8 + 4, 140, 4+currentTime.Hour*8 + 4, 146, FOREGROUND_COLOR);

    // Show number of steps as text and bitmap
    int32_t step_count_day = getStepsOfDay();
    UiSDK::fillRect(display, 0, 170, 196, 26, FOREGROUND_COLOR);

    UiSDK::setTextColor(display, BACKGROUND_COLOR);
    UiSDK::setFont(display, &FONT_MEDUM_BOLD);
    UiSDK::getTextBounds(display, String(step_count_day), 55, 195, &x1, &y1, &width, &height);
    int8_t bitmap_pos = int(200-width)/2;
    UiSDK::drawBitmap(display, bitmap_pos-15, 174, steps, 19, 23, BACKGROUND_COLOR);
    UiSDK::setCursor(display, bitmap_pos+15, 192);
    UiSDK::println(display, step_count_day);
