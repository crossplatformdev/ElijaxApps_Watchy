...
void WatchyBTTF::drawWatchFace(){
    // Tie the BCD suite's local dark-mode flag to the global ThemeApp setting.
    bcd_dark_mode = gDarkMode;
    ...
    UiSDK::drawBitmap(display, 0, 0, bttf, 200, 200, FOREGROUND_COLOR);
    drawTime();
    drawDate();
    drawSteps();
    drawBattery();
...
void WatchyBTTF::drawTime(){
    UiSDK::setFont(display, &FONT_7_SEG_LARGE);
...
void WatchyBTTF::drawDate(){
    UiSDK::setFont(display, &FONT_7_SEG_MEDIUM);
    UiSDK::setTextColor(display, BACKGROUND_COLOR);
    ...
    printCentered(94, 103, dayStr);
        ...
        printCentered(157, 103, yearStr);
            ...
            String alarm_str = "T-" + String(bcd_alarm_timer);
