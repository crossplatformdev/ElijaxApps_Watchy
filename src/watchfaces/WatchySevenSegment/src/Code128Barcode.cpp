...
void Code128Barcode::draw() {
    ...
    display->fillRect(0,0,display->width(), display->height(), bgColor); // clear the screen
    for (int i = 0; i < encodedBarcode.length(); i++) {
    if (encodedBarcode[i] == '1') {
        display->fillRect(xStart + (i * barWidth), yStart, barWidth, height, fgColor);
    } else {
        display->fillRect(xStart + (i * barWidth), yStart, barWidth, height, bgColor);
    }
    }
    ...
    display->fillRect(0,0,textWidth,textHeight,bgColor);
    ...
    display->fillRect(0,display->height()-textHeight,textWidth,textHeight,bgColor);
