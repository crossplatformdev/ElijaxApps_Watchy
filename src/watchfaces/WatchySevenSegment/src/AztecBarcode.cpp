...
void AztecBarcode::draw(){
    ...
    m_display->setTextColor(fgColor);
    m_display->getTextBounds(m_label.c_str(),0,0,&x1,&y1,&textWidth,&textHeight);
    m_display->fillRect(0,0,textWidth,textHeight,bgColor);
    ...
    for (int i = 0; i < AZTEC_DIMENSION; i++) {
        for (int j = 0; j < AZTEC_DIMENSION; j++) {
            if (m_matrix[i][j]) {
                m_display->fillRect(xoffset + i*pixelSize,yoffset + j*pixelSize,pixelSize,pixelSize,fgColor);
            } 
    ...
    m_display->fillRect(0,m_display->height()-textHeight,textWidth,textHeight,bgColor);
