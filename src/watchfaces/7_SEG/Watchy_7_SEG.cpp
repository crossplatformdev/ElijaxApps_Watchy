...
void Watchy7SEG::drawWatchFace(Watchy &watch){
  ...
  drawHeartRate(watch);
    ...
    #endif
    // Mark layout boxes as dirty so the SDK can perform targeted partial
    // updates instead of doing a full refresh.
    for (size_t i = 0; i < sizeof(layoutBoxes) / sizeof(layoutBoxes[0]); ++i) {
      const LayoutBox &b = layoutBoxes[i];
      WatchyUi::Screen::invalidate(WatchyUi::Bounds{
        static_cast<int16_t>(b.left), static_cast<int16_t>(b.top),
        static_cast<int16_t>(b.right - b.left),
        static_cast<int16_t>(b.bottom - b.top)});
    }
}
...
void Watchy7SEG::drawHeartRate(Watchy &) {
    ...
    String bpmText(watchfaceHeartRateBpm());
    if (!drawTextInBox(bpmText.c_str(), bpmBox, 193, false)) {
      Watchy::display.setFont();
      WatchyUi::Canvas::centeredText(
          {bpmBox.left, bpmBox.top, bpmBox.right - bpmBox.left,
           bpmBox.bottom - bpmBox.top}, bpmText.c_str(),
          WatchyUi::Theme::foreground());
    }
}

  void Watchy7SEG::refreshHeartRate(Watchy &watch) {
    // Redraw only the heart-rate bounds to avoid unnecessary work and
    // minimize the number of pixels refreshed.
    int16_t w = static_cast<int16_t>(heartRateBox.right - heartRateBox.left);
    int16_t h = static_cast<int16_t>(heartRateBox.bottom - heartRateBox.top);
        drawHeartRate(watch);
    WatchyUi::Screen::present({heartRateBox.left, heartRateBox.top, w, h},
                  WATCHFACE_STATE);
  }
...
void Watchy7SEG::drawSteps(Watchy &){
    ...
    String stepText;
    if (stepCount < 10000) {
      stepText = String(static_cast<unsigned long>(stepCount));
      while (stepText.length() < 4) {
        stepText = String("0") + stepText;
      }
    } else {
      ...
      stepText = String(static_cast<unsigned long>(thousands)) + "K";
    }
    drawTextInBox(stepText.c_str(), stepTextBox, 190, true);
}
...
void Watchy7SEG::drawBattery(Watchy &watch){
  Watchy::display.drawBitmap(batteryBox.left, batteryBox.top, battery,
           ...
           WatchyUi::Theme::foreground());
  Watchy::display.fillRect(163, 78, 27, BATTERY_SEGMENT_HEIGHT, WatchyUi::Theme::background());//clear battery segments
  #ifdef WATCHY_DETERMINISTIC_GALLERY
    ...
    for(int8_t batterySegments = 0; batterySegments < batteryLevel; batterySegments++){
        Watchy::display.fillRect(163 + (batterySegments * BATTERY_SEGMENT_SPACING), 78, BATTERY_SEGMENT_WIDTH, BATTERY_SEGMENT_HEIGHT, WatchyUi::Theme::foreground());
    }
}
...
void Watchy7SEG::drawWeather(Watchy &watch){
    ...
    Watchy::display.drawBitmap(unitBox.left, unitBox.top,
           weatherSnapshot.isMetric ? celsius : fahrenheit,
