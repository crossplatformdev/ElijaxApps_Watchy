...
The checked-in screenshots were recaptured from the Watchy and visually
reviewed on 2026-09-16 after the app lifecycle and shared layout fixes.
See [current verification](docs/readme.md#current-verification) for build,
test, catalog, and hardware-validation status.
...
In **Pong**, tap or hold UP/DOWN to move the player paddle. A short press is
retained until the next game frame; held input moves it by 12 pixels per
frame, clamped to the field. MENU pauses/resumes or starts a new game after
game over, BACK returns to the menu, and the first side to score five wins.
The simulation waits at least 120 ms between completed frames; e-paper
refresh and vibration add to that interval, so this is not a guaranteed FPS.

In **Snake**, UP turns left and DOWN turns right relative to travel, with at
most one queued turn per move. Eating fruit adds one segment and one point.
Hitting a wall or any occupied body cell, including the current tail, ends
the game before moving the body. MENU pauses/resumes or restarts after game
over; BACK exits. Moves wait at least 180 ms after the previous redraw.
...
| **Time Beacon** | Advertises RTC-derived UTC time in a service payload. | ![Time Beacon - on air](docs/gallery/png/bluetooth/time-beacon/on-air/light.png) | ![Time Beacon - paused](docs/gallery/png/bluetooth/time-beacon/paused/light.png) |  |  |  |
...
If `pio` is not on PATH on Windows, invoke PlatformIO's managed environment:
...
build. It writes no telemetry to NVS/Flash. Debug messages use `@WATCHY_POWER`
at 115200 baud, but structured counter summaries are currently blocked:
startup does not call `WatchyDiagnostics::beginWake()`, which initializes wake
accounting and prints the previous counters. Do not treat debug messages as
valid counter captures until that lifecycle hook is restored.
...
As recorded on 2026-09-16, the host runner reports 75 tests run, one failure,
and two skips. The BCG replay class setup and metronome timing test require a
host C++ compiler; the strict-warning configuration test still fails. The catalog
check passes using
[src/sdk/demo/GalleryAppCatalog.inc](src/sdk/demo/GalleryAppCatalog.inc).
All 348 scenes were regenerated from device framebuffers and their PNG/PBM
hashes verified. All 16 watchface images match unchanged golden hashes.
Production and gallery builds pass. Physical button interaction and live
network/sensor workflows still need hands-on acceptance; see
[the lifecycle review](docs/readme.md#app-lifecycle-review).
...
Catalog regeneration is blocked by the source-path mismatch noted above.
The capture commands use the existing scene-ID catalog; do not publish a new
capture as synchronized until that check is repaired and passes.
