...
class ReconnectingSerial:
    def __init__(self, serial_module, port: str, baud: int,
                 read_timeout: float, reconnect_timeout: float):
        ...
        self.stream = None
        self._reopen(time.monotonic() + reconnect_timeout)
    ...
    def _reopen(self, deadline: float) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                self.stream = self.serial_module.Serial(
                    self.port, self.baud, timeout=self.read_timeout
                )
                return
            except (OSError, self.serial_module.SerialException) as error:
                last_error = error
                time.sleep(0.1)
        raise CaptureError(
            f"could not reopen serial port {self.port}: {last_error}"
        )
    ...
    def _with_reconnect(self, operation):
        ...
        while True:
            try:
                return operation(self.stream)
            except (OSError, self.serial_module.SerialException):
                self._reopen(deadline)

    def read(self, size: int) -> bytes:
        return self._with_reconnect(lambda stream: stream.read(size))

    def write(self, data: bytes) -> int:
        return self._with_reconnect(lambda stream: stream.write(data))

    def flush(self) -> None:
        self._with_reconnect(lambda stream: stream.flush())
    ...
    try:
        ...
        if args.input is not None:
            ...
            with ReconnectingSerial(
                serial, args.port, args.baud, min(args.timeout, 0.25), args.timeout
            ) as stream:
