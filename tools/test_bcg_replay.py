...
class BcgReplayTests(unittest.TestCase):
    ...
    def make_trace(self, sample_rate_hz=25, seconds=90, bpm=60,
        ...
        with path.open("w", encoding="ascii", newline="\n") as stream:
            ...
            for index in range(sample_rate_hz * seconds):
                ...
                noise = alternating_noise if index % 2 else -alternating_noise
                stream.write(f"{index},0,0,{1024 + pulse + noise}\n")
        return path
    ...
    def replay(self, trace, target_rate_millihz):
        ...
        return {
            "valid_bpms": [
                int(line.split()[5]) for line in lines
                if line.startswith("@WATCHY_BCG_WINDOW") and line.split()[4] == "1"
            ],
            "rate": int(fields[2]),
    ...
    def test_detects_single_count_weak_pulses(self):
        trace = self.make_trace(pulse_shape=(0, 0, 0, 1))
        result = self.replay(trace, 25000)
        self.assertGreaterEqual(result["beats"], 80)
        self.assertLessEqual(result["beats"], 90)
        self.assertEqual(result["valid_windows"], 6)
        self.assertTrue(all(abs(bpm - 60) <= 1 for bpm in result["valid_bpms"]))

    def test_constant_signal_has_no_beats(self):
        trace = self.make_trace(pulse_shape=())
        result = self.replay(trace, 25000)
        self.assertEqual(result["beats"], 0)
        self.assertEqual(result["valid_windows"], 0)

    def test_alternating_single_count_noise_has_no_valid_windows(self):
        trace = self.make_trace(pulse_shape=(), alternating_noise=1)
        result = self.replay(trace, 25000)
        self.assertEqual(result["valid_windows"], 0)
    ...
    def test_rolling_ten_beat_average(self):
        result = subprocess.run(
        ...
        self.assertIn("Rolling ten-beat checks passed", result.stdout)
