...
class CaptureGalleryTests(unittest.TestCase):
    def test_static_utilities_and_os_results_wait_for_back(self):
        repository = Path(__file__).resolve().parents[1]
        utility_path = repository / "src/sdk/utilities"
        support = (utility_path / "UtilitySupport.cpp").read_text()
        runtime = support.split("void runStaticView(", 1)[1].split(
            "void runGenerator(", 1
        )[0]
        self.assertLess(runtime.index("Input::begin()"), runtime.index("renderer();"))
        self.assertIn("while (WatchyUi::Input::wait() != WatchyUi::Event::BACK)", runtime)
        for name in ("ChipInfo", "HeapMonitor", "I2cScanner", "WakeReason", "ResetCause"):
            source = (utility_path / f"{name}.cpp").read_text()
            runtime, preview = source.split("#ifdef WATCHY_DETERMINISTIC_GALLERY", 1)
            with self.subTest(utility=name):
                self.assertIn("runStaticView(", runtime)
                self.assertNotIn("runStaticView(", preview)
                self.assertNotIn("Input::wait", preview)
        for name in ("ShowBuzz", "ShowSyncNTP"):
            source = (repository / f"src/sdk/os/{name}.cpp").read_text()
            with self.subTest(os_app=name):
                self.assertIn("WatchyUi::Input::begin();", source)
                self.assertIn("while (WatchyUi::Input::wait() != WatchyUi::Event::BACK)", source)
                self.assertNotIn("deepSleepDelay(3000)", source)
        heart_rate = (repository / "src/app/ShowHeartRate.cpp").read_text()
        failure = heart_rate.split('"Measurement could not start"', 1)[1].split(
            "void Watchy::showHeartRate", 1
        )[0]
        self.assertIn("#ifndef WATCHY_BCG_TRACE_CAPTURE", failure)
        self.assertIn("while (WatchyUi::Input::wait() != WatchyUi::Event::BACK)", failure)

    def test_screen_ruler_waits_for_back_without_blocking_preview(self):
        repository = Path(__file__).resolve().parents[1]
        source = (
            repository / "src/sdk/utilities/ScreenRuler.cpp"
        ).read_text()
        runtime = source.split("void runScreenRuler() {", 1)[1].split(
            "#ifdef WATCHY_DETERMINISTIC_GALLERY", 1
        )[0]
        self.assertLess(runtime.index("Input::begin()"), runtime.index("draw();"))
        self.assertLess(runtime.index("draw();"), runtime.index("while (true)"))
        self.assertIn("WatchyUi::Event event = WatchyUi::Input::wait();", runtime)
        self.assertIn("if (event == WatchyUi::Event::BACK) return;", runtime)
        self.assertEqual(runtime.count("return;"), 1)
        self.assertEqual(runtime.count("draw();"), 1)
        self.assertIn("void renderScreenRulerPreview(uint8_t) { draw(); }", source)

    def test_renderers_centralize_canvas_initialization(self):
        repository = Path(__file__).resolve().parents[1]
        for directory in (
            "src/app", "src/sdk/os", "src/sdk/tools", "src/sdk/utilities",
            "src/sdk/include/App", "src/watchfaces",
        ):
            for source in (repository / directory).rglob("*"):
                ...
                with self.subTest(source=source.relative_to(repository)):
                    self.assertNotRegex(
                        source.read_text(), r"\b(?:setFullWindow|fillScreen)\s*\("
                    )
                    self.assertNotRegex(
                        source.read_text(),
                        r"\bdisplay\s*\.\s*(?:display|displayWindow|refresh|"
                        r"firstPage|nextPage)\s*\(",
                    )
                    self.assertNotRegex(
                        source.read_text(),
                        r"\b(?:digitalRead|gpio_get_level)\s*\(\s*"
                        r"(?:MENU|BACK|UP|DOWN)_BTN_PIN\s*\)",
                    )
                    self.assertNotRegex(source.read_text(), r"\bdisplay\s*\.\s*epd2\b")
        registry = (repository / "src/sdk/WatchfaceSelector.cpp").read_text()
        for signature in (
            "void drawWatchFace() {",
            "void Watchy::drawWatchFace() {",
            "void drawGalleryWatchface(uint8_t watchfaceId, const tmElements_t &fixedTime) {",
        ):
            self.assertIn(signature + "\n  WatchyUi::Screen::beginCanvas();", registry)
        ui = (repository / "src/sdk/WatchyUi.cpp").read_text()
        self.assertIn('void Screen::begin(const char *title) {\n  beginCanvas();', ui)
        app_display = (repository / "src/sdk/include/App/AppDisplay.h").read_text()
        self.assertIn("WatchyUi::Screen::begin(title);", app_display)
        self.assertIn("WatchyUi::Screen::present();", app_display)
        menu = (repository / "src/sdk/os/ShowMenu.cpp").read_text()
        self.assertIn("WatchyUi::ListView::draw(model);", menu)
        self.assertIn("WatchyUi::ListView::presentSelectionChange", menu)
        face = (repository / "src/sdk/os/ShowWatchFace.cpp").read_text()
        self.assertIn("WatchyUi::Screen::present(WATCHFACE_STATE, false);", face)
        charge = (repository / "src/sdk/tools/Sensors/ChargeStatus.cpp").read_text()
        self.assertIn("WatchyUi::Power::usbPluggedIn()", charge)
        self.assertNotIn("digitalRead(USB_DET_PIN)", charge)
    ...
    def test_back_interrupts_application_delays(self):
        ...
        self.assertIn("digitalRead(BACK_BTN_PIN) == ACTIVE_LOW", delay)
        self.assertIn("delayedBackPending = true;", delay)
        self.assertIn("return WakeupReason::BACK_PRESSED;", delay)
        self.assertIn("min<uint32_t>(remaining, 10)", delay)
        self.assertNotIn("esp_light_sleep_start", delay)
        core = (repository / "src/sdk/WatchyCore.cpp").read_text()
        button_wake = core.split("void handleButtonWake(uint64_t wakeupBits) {", 1)[1].split(
            "void handleUsbWake", 1
        )[0]
        ...
        self.assertIn("eventFromWakeBits(wakeupBits & BTN_PIN_MASK)", button_wake)
        self.assertIn("handleButtonEvent(event);", button_wake)
        ...
        self.assertIn("if (delayedBackPending)", poll)
        self.assertIn("delayedBackPending = false;", poll)
        self.assertIn("delayedBackTimeoutMs", poll)
        self.assertIn("buttons & ~previousButtons", poll)
        self.assertIn("constexpr uint32_t buttonDebounceMs = 5;", ui)
        self.assertIn("pdMS_TO_TICKS(buttonDebounceMs)", poll)
        self.assertIn("if (buttons != readButtons()) return Event::NONE;", poll)
    ...
    def test_usb_presence_blocks_sleep_and_keeps_buttons_serviced(self):
        ...
        usb_wake = core.split("bool configureUsbWakeForDeepSleep()", 1)[1].split(
            "bool configureButtonWakeForDeepSleep()", 1
        )[0]
        self.assertIn("static_cast<gpio_num_t>(USB_DET_PIN)", usb_wake)
        self.assertIn("configureRtcWakeInput(pin)", usb_wake)
        self.assertIn("rtc_gpio_get_level(pin) == wakeLevel", usb_wake)
        ...
        self.assertIn("for (;;)", deep_sleep)
        self.assertIn("Input::waitScheduled(100)", deep_sleep)
        self.assertNotIn("return;", deep_sleep.split("void handleButtonPress", 1)[0])
        button_wake = core.split("bool configureButtonWakeForDeepSleep()", 1)[1].split(
            "bool configureMinuteTimerWakeForDeepSleep()", 1
        )[0]
        self.assertLess(button_wake.index("rtc_gpio_get_level"),
                        button_wake.index("esp_sleep_enable_ext1_wakeup"))
    ...
    def test_pong_preserves_direction_events_and_supports_held_buttons(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/app/ShowPong.cpp").read_text()
        loop = source.split("void showPongImpl(Watchy *watchy)", 1)[1].split(
            "void Watchy::showPong()", 1
        )[0]
        self.assertNotIn("Input::poll()", loop)
        self.assertIn("queuedDirection = event;", loop)
        self.assertLess(loop.index("queuedDirection = event;"),
                        loop.index("movePlayerPaddle(state, queuedDirection,"))
        self.assertIn("WatchyUi::Input::pressed(WatchyUi::Event::UP)", loop)
        self.assertIn("WatchyUi::Input::pressed(WatchyUi::Event::DOWN)", loop)
        ui = (repository / "src/sdk/WatchyUi.cpp").read_text()
        held = ui.split("bool Input::pressed(Event event)", 1)[1].split(
            "Event Input::wait(", 1
        )[0]
        for button in ("MENU", "BACK", "UP", "DOWN"):
            self.assertIn(
                f"case Event::{button}: return digitalRead({button}_BTN_PIN) == ACTIVE_LOW;",
                held,
            )
        self.assertIn("default: return false;", held)
        self.assertNotIn("previousButtons", held)
        self.assertNotIn("poll()", held)
        frame = loop.split("movePlayerPaddle(state, queuedDirection,", 1)[1]
        self.assertLess(frame.index("queuedDirection = WatchyUi::Event::NONE;"),
                        frame.index("updatePong(state)"))
        movement = source.split("void movePlayerPaddle", 1)[1].split(
            "void resetBall", 1
        )[0]
        self.assertIn("if (state.paused || state.gameOver) return;", movement)
        self.assertIn("WatchyUi::Event::UP) direction = -1;", movement)
        self.assertIn("WatchyUi::Event::DOWN) direction = 1;", movement)
        self.assertIn("upHeld != downHeld", movement)
        self.assertIn("clampPaddleY(state.playerY + direction * 12)", movement)

    def test_snake_collision_includes_tail_before_moving_body(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/app/ShowSnake.cpp").read_text()
        movement = source.split("MoveResult moveSnake(SnakeState &state)", 1)[1]
        collision = movement.split(
            "if (snakeOccupies(state, nextX, nextY, state.length)) {", 1
        )[1].split("}", 1)[0]
        self.assertIn("state.gameOver = true;", collision)
        self.assertIn("state.paused = true;", collision)
        self.assertIn("return COLLIDED;", collision)
        self.assertLess(movement.index("snakeOccupies("),
                        movement.index("state.bodyX[segment] ="))
    ...
    def test_bcg_refreshes_only_pixel_changed_heart_rate_region(self):
        ...
        self.assertIn("previousBpm != watchfaceHeartRateBpm()", heart_rate)
    ...
    def test_all_watchfaces_show_retained_rolling_bpm_without_placeholder(self):
        repository = Path(__file__).resolve().parents[1]
        ...
        process = source.split("WatchyBcg::Result processWatchfaceSample", 1)[1].split(
            "void drawAppHeartRate", 1
        )[0]
        self.assertIn("watchfaceSampleRateMilliHz, WatchyBcg::ResultMode::Live", process)
        self.assertIn("if (result.valid) lastValidHeartRateBpm = result.bpm;", process)
        reset = source.split("void resetWatchfaceState", 1)[1].split(
            "bool recoverWatchfaceFifo", 1
        )[0]
        self.assertNotIn("lastValidHeartRateBpm", reset)
        self.assertIn("RTC_DATA_ATTR uint16_t lastValidHeartRateBpm = 0;", source)
        self.assertIn("return lastValidHeartRateBpm;", source)
        self.assertEqual(registry.count("drawActiveWatchface(watch);"), 2)
        self.assertIn("drawActiveWatchface(*this);", registry)
        draw = registry.split("void drawActiveWatchface", 1)[1].split(
            "} // namespace", 1
        )[0]
        self.assertIn("activeWatchfaceOps().draw(watch);", draw)
        self.assertIn("drawGenericHeartRate();", draw)
        self.assertEqual(registry.count("noDataUpdate, genericRefreshHeartRate}"), 7)
        generic = registry.split("void drawGenericHeartRate()", 1)[1].split(
            "struct WatchfaceOps", 1
        )[0]
        self.assertIn("watchfaceHeartRateBpm()", generic)
        self.assertNotIn('"--"', generic)
        self.assertIn("Screen::presentDirty(WATCHFACE_STATE)", generic)
        full = seven_segment.split("void Watchy7SEG::drawWatchFace", 1)[1].split(
            "void Watchy7SEG::drawHeartRate", 1
        )[0]
        self.assertIn("drawHeartRate(watch);", full)
        heart = seven_segment.split("void Watchy7SEG::drawHeartRate", 1)[1].split(
            "void Watchy7SEG::drawTime", 1
        )[0]
        self.assertIn("watchfaceHeartRateBpm()", heart)
        self.assertIn("drawHeartRate(watch);", heart)
        self.assertNotIn('"--"', heart)
    ...
    def test_dirty_refresh_never_expands_to_an_incomplete_full_frame(self):
        ...
        self.assertIn("displayWindow(bounds.x, bounds.y,", present_dirty)
        self.assertNotIn("display(false)", present_dirty)
        self.assertNotIn("fullRefreshDue(", present_dirty)

    def test_full_frame_present_does_not_skip_direct_gfx_drawing(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/sdk/WatchyUi.cpp").read_text()
        present = source.split("void Screen::present(int", 1)[1].split(
            "void Screen::present(const Bounds", 1
        )[0]
        self.assertIn("Watchy::ensureDisplayInitialized();", present)
        self.assertIn("Watchy::display.display(!fullRefresh);", present)
        self.assertNotIn("presentDirty(", present)
        self.assertNotIn("recordSkippedDisplayUpdate", present)
        wrapper = (repository / "src/Watchy.cpp").read_text()
        self.assertIn("Watchy::display =\n    WatchySdk::Device::display;", wrapper)
        self.assertIn("&Watchy::RTC = WatchySdk::Device::RTC;", wrapper)

    def test_canvas_and_widgets_do_not_send_intermediate_frames(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/sdk/WatchyUi.cpp").read_text()
        canvas = source.split("void Screen::beginCanvas()", 1)[1].split(
            "void Screen::begin(", 1
        )[0]
        widgets = source.split("void Widget::bodyText", 1)[1].split(
            "void ValueView::draw", 1
        )[0]
        self.assertNotIn(".display(", canvas)
        self.assertNotIn(".display(", widgets)

    def test_driver_compares_framebuffer_before_partial_transfers(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/Display.cpp").read_text()
        self.assertIn("lastFrame[offset] != bitmap[offset]", source)
        self.assertIn("changedLeft * 8", source)
        self.assertEqual(source.count("compareFrame(bitmap, x, y, w, h);"), 2)
        refresh = source.split("void WatchyDisplay::refresh(int16_t", 1)[1].split(
            "void WatchyDisplay::powerOff", 1
        )[0]
        self.assertIn("if (frameDiff.width == 0)", refresh)
        self.assertLess(refresh.index("recordSkippedDisplayUpdate"),
                        refresh.index("_Update_Part();"))
        self.assertIn("w = frameDiff.width;", refresh)
        self.assertIn("h = frameDiff.height;", refresh)
        again = source.split("void WatchyDisplay::writeImageAgain", 1)[1].split(
            "void WatchyDisplay::_writeImage", 1
        )[0]
        self.assertIn("if (frameDiff.width != 0)", again)
        self.assertIn("frameDiff.width, frameDiff.height", again)
        for method in ("hibernate()", "_reset()", "writeScreenBuffer(uint8_t value)"):
            body = source.split("void WatchyDisplay::" + method, 1)[1]
            self.assertIn("invalidateFrameReference();", body.split("}", 1)[0])
    ...
    def test_foreground_heart_rate_keeps_scheduler_running(self):
        repository = Path(__file__).resolve().parents[1]
        heart_rate = (repository / "src/app/ShowHeartRate.cpp").read_text()
        header = (repository / "src/sdk/include/App/HeartRate.h").read_text()
        processor = (repository / "src/app/BcgProcessor.cpp").read_text()
        processor_header = (
            repository / "src/sdk/include/App/BcgProcessor.h"
        ).read_text()
        measurement = heart_rate.split("bool runHeartRateMeasurement(", 1)[1].split(
            "void heartRateTask", 1
        )[0]
        self.assertNotIn("durationMs", heart_rate)
        self.assertNotIn("durationMs", header)
        self.assertIn("while (!stopRequested())", measurement)
        self.assertIn("sleepUntilSample(now, nextSampleAt)", measurement)
        self.assertIn("nextSampleAt = now + samplePeriodMs;", measurement)
        self.assertIn("WatchyBcg::ResultMode::Live", measurement)
        self.assertIn("measureHeartRateSilently(queueAppHeartRateUpdate)", heart_rate)
        self.assertIn("ResultMode mode = ResultMode::Windowed", processor_header)
        live = processor.split("if (mode == ResultMode::Live)", 1)[1].split(
            "state.windowSampleCount++;", 1
        )[0]
        self.assertIn("return currentResult(state, estimateUpdated, beatDetected)", live)
        self.assertIn("averageBeatCount = 10", processor_header)
        self.assertIn("state.intervalSamples[state.nextIntervalIndex] = interval", processor)
        self.assertIn("state.intervalSampleSum -= state.intervalSamples", processor)
        self.assertIn("if (signalExpired)", processor)
        self.assertIn("state.hasBeat = false;", processor)
        self.assertNotIn("windowSamples", live)
        ui = (repository / "src/sdk/WatchyUi.cpp").read_text()
    ...
    def test_sync_ntp_always_stops_radios_before_lowering_cpu(self):
        repository = Path(__file__).resolve().parents[1]
        for relative, signature in (
            ("src/sdk/os/ShowSyncNTP.cpp", "bool Watchy::syncNTP(long gmt, String ntpServer)"),
            ("src/sdk/WatchyCore.cpp", "bool syncNTP(long gmt, String ntpServer)"),
        ):
            with self.subTest(source=relative):
                source = (repository / relative).read_text()
                body = source.split(signature, 1)[1].split("\n}", 1)[0]
                self.assertIn("bool synced = timeClient.forceUpdate();", body)
                self.assertIn("if (synced)", body)
                self.assertEqual(body.count("return "), 1)
                operations = (
                    "RTC.set(tm);", "timeClient.end();", "WiFi.mode(WIFI_OFF);",
                    "btStop();", "WatchyDiagnostics::endWifiSession();",
                    "Watchy::setLowPowerCpuFrequency();", "return synced;",
                )
                positions = [body.index(operation) for operation in operations]
                self.assertEqual(positions, sorted(positions))
        wrapper = (repository / "src/Watchy.cpp").read_text()
        self.assertIn("LOW_POWER_CPU_FREQUENCY_MHZ = 8;", wrapper)
        self.assertIn("setCpuFrequencyMhz(LOW_POWER_CPU_FREQUENCY_MHZ);", wrapper)
    ...
    def test_watchfaces_match_independent_pixel_golden_hashes(self):
        repository = Path(__file__).resolve().parents[1]
        source = (repository / "src/watchfaces/7_SEG/Watchy_7_SEG.cpp").read_text()
        self.assertIn("weatherSnapshot.isMetric ? celsius : fahrenheit", source)
        manifest = json.loads(
