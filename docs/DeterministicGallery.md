...
renderers. It covers all 142 registered application actions plus the root menu
and ten category menus, for 153 scenes in total. The default firmware and its
display behavior are unchanged.
...
The ordered contract is stored in `tools/gallery_scene_ids.txt`. Unit tests
compare it directly with the scene arrays in the firmware. The receiver fails
unless all 153 expected IDs arrive exactly once and in order, each at 200x200
with 5,000 bytes and a valid CRC32, followed by matching end and completion
records.
...
```powershell
python -m unittest tools/test_capture_gallery.py
```

The tests exercise the handshake, framing, CRC rejection, exact catalog order,
PBM inversion, PNG creation, and manifest generation. They do not open or
