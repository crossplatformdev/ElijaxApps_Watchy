...
Disable Fall Monitor and explicitly disable watch-face BCG monitoring during
each trial so neither requests a different sensor mode. Selecting a non-7-SEG
face is insufficient: cold boot now requests BCG on every face. There is no
documented on-watch BCG toggle in the current UI. A controlled trial build must
disable the `setWatchfaceHeartRateMonitoring(true)` request in
[WatchyCore.cpp](../src/sdk/WatchyCore.cpp), verify Baseline ownership, and
apply the same change to both ODR builds. This is a trial prerequisite, not a
change made by the `step-odr-25` environment itself. For both builds:
