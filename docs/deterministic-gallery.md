...
scenes in total. The gallery profile uses the shared renderers but bypasses
normal startup and panel I/O; it is not a physical display test.

## Status

The checked-in catalog currently contains 142 applications and 348 ordered
scenes. The source is
[GalleryAppCatalog.inc](../src/sdk/demo/GalleryAppCatalog.inc).
On 2026-09-16 the catalog CLI was corrected and its check passes. All 348
scenes were captured from a connected Watchy and published with validated
checksums and file hashes. Contact sheets covering every scene and detailed
views of corrected screens were reviewed. Long headings, oversized values,
empty-state details, footers, and the Heart Rate caption were improved.

The 7-SEG unit icon now uses the same weather snapshot as its numeric value.
All 16 watchface captures match the existing golden hashes without updating
the baseline. Production firmware was restored after capture. These fixtures
exercise renderers, not physical button interaction, panel I/O, or live services.
...
On Windows, an editor or image viewer can block the final directory rename.
The current `--replace` implementation removes the old output before that
rename, so do not rely on it as a rollback mechanism. For a directory being
viewed, capture to a new temporary output first, then copy the validated output
into `docs/gallery` and verify PNG/PBM hashes against its manifest. This was
necessary for the final 2026-09-16 capture.
...
`src/sdk/demo/GalleryAppCatalog.inc`.
`tools/sync_gallery_catalog.py` derives the
...
For the remaining strict-warning configuration failure, skipped native tests,
and hands-on acceptance checklist, see
[Current Verification](readme.md#current-verification).