...
Bounded background capture and host export are implemented. Background
classification, alerting, emergency broadcasts, and threshold promotion remain
disabled pending labeled traces. The separate foreground live detector does
implement low-g, impact, and stillness states and vibrates on its `Alert`
state; it is experimental and is not a validated fall alarm.
...
Open **Safety & Aid > Fall Detector**:

- **MENU** (shown as `SEL` on screen) toggles background candidate logging.
- **UP** opens the original foreground live detector.
- **DOWN** confirms and clears all stored calibration traces.
- **BACK** returns to the Safety & Aid menu.
...
2. Open Fall Detector and press MENU until it shows `ARMED`.
