...
Deadline arithmetic and worker ownership have deterministic host tests. On
2026-09-15 the native timing test is skipped because no supported host C++
compiler is available; earlier arithmetic results below are historical, not
a fresh execution. See [Current Verification](readme.md#current-verification)
for firmware-build status. Physical GPIO onset, motor response, and long-run
on-watch jitter remain pending.
...
The previously recorded result covers 211 tempos with `phase_error_us=0`, `skip=ok`, and
`accent=ok`.
...
This protocol is currently blocked by missing startup integration:
`WatchyDiagnostics::beginWake()` has no caller, so the previous-wake numeric
counter summary is not emitted. Restore that hook and verify a valid counter
record before collecting timing results. Do not interpret ordinary
`@WATCHY_POWER` debug lines as metronome measurements.
