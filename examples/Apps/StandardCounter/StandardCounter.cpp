...
void editNumber(Watchy &watch, CounterRecord &record, bool editingStep) {
...
void resetCounter(Watchy &watch, CounterRecord &record) {
...
void run(Watchy &watch) {
  ...
  while (true) {
    ...
    if (event == WatchyUi::Event::UP) {
      ...
      if (selected < 2) editNumber(watch, record, selected == 1);
      else resetCounter(watch, record);
    }
  }
}
